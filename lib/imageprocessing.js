(function() {
  ...
  ImageProcessing = {
    ...
    stretch: function(astropng, vmin, vmax, stretchFunction, canvasArr) {
      ...
      softening = this.softening[astropng.header['BAND']] || 1;
      stretchFunction = stretchFunction != null ? stretchFunction : 'linear';
      numberOfPixels = imageData.length;
      length = 4 * numberOfPixels;
      min = vmin != null ? this.normalize(vmin, softening) : this.normalize(astropng.vmin, softening);
      max = vmax != null ? this.normalize(vmax, softening) : this.normalize(astropng.vmax, softening);
    ...
    stretchLine: function(pixels, vmin, vmax, stretchFunction, canvasArr) {
      var i, index, length, max, min, numberOfPixels, pixel, range, _ref;
      stretchFunction = stretchFunction != null ? stretchFunction : 'linear';
      numberOfPixels = pixels.length;
      length = 4 * numberOfPixels;
      min = this.normalize(vmin, 1);
      max = this.normalize(vmax, 1);
      range = max - min;
      for (i = 0, _ref = length - 1; i <= _ref; i += 4) {
        index = i / 4;
        pixel = this.scale(pixels[index], min, range);
        pixel = this.normalize(pixel, 1, stretchFunction);
        pixel = this.scaleToColorSpace(pixel);
        canvasArr[i + 0] = pixel;
        canvasArr[i + 1] = pixel;
        canvasArr[i + 2] = pixel;
