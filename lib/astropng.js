(function() {
  ...
  AstroPNG = (function() {
    ...
    function AstroPNG(buffer) {
      this.computeStatistics = __bind(this.computeStatistics, this);
      this.readImageData = __bind(this.readImageData, this);
      ...
      this.evt.initEvent('readLine', true, true);
    ...
    AstroPNG.randomNumberGenerator = function(n) {
      ...
      for (i = 0; 0 <= n ? i <= n : i >= n; 0 <= n ? i++ : i--) {
        temp = a * this.seed;
        this.seed = temp - m * Math.floor(temp / m);
        randomNumbers.push(this.seed / m);
      }
    ...
    */
    ...
    AstroPNG.prototype.readIHDR = function(length) {
      ...
      this.evt.numberOfLines = this.height;
      allowedColorTypes = [0];
    ...
    AstroPNG.prototype.readFitsHeader = function(length) {
      var c, card, cards, data, key, value, _i, _len;
      if (length === 0) return;
      ...
      if (this.header.hasOwnProperty('MINPIXEL')) {
        this.minimumPixel = this.vmin = parseFloat(this.header['MINPIXEL']);
      }
      if (this.header.hasOwnProperty('MAXPIXEL')) {
        return this.maximumPixel = this.vmax = parseFloat(this.header['MAXPIXEL']);
      }
    };
    ...
    AstroPNG.prototype.readQuantizationParameters = function(length) {
      ...
      if (length === 0) return;
      length /= 4;
      return this.quantizationParameters = (function() {
        var _results;
        _results = [];
        for (i = 1; 1 <= length ? i <= length : i >= length; 1 <= length ? i++ : i--) {
    ...
    AstroPNG.prototype.readLine = function() {
      ...
      if (this.quantizationParameters != null) {
        ...
        r = AstroPNG.randomNumberGenerator(this.width);
      ...
      this.evt.currentLine = this.currentLine;
    ...
    AstroPNG.prototype.readImageData = function() {
      ...
      for (j = 1, _ref = this.height; 1 <= _ref ? j <= _ref : j >= _ref; 1 <= _ref ? j++ : j--) {
        _results.push(this.readLine());
      }
      return _results;
    };

    AstroPNG.prototype.computeStatistics = function() {
      var diff2, i, imageData, maxsInRow, minsInRow, pixel, sum, _i, _j, _len, _len2;
      ...
      minsInRow = (function() {
        var _ref, _results;
        _results = [];
        for (i = 1, _ref = this.height; 1 <= _ref ? i <= _ref : i >= _ref; 1 <= _ref ? i++ : i--) {
          _results.push(Math.min.apply(Math, imageData.slice(i * this.width, (i * this.width + this.width) + 1 || 9e9)));
        }
        return _results;
      }).call(this);
      maxsInRow = (function() {
        ...
        _results = [];
        for (i = 1, _ref = this.height; 1 <= _ref ? i <= _ref : i >= _ref; 1 <= _ref ? i++ : i--) {
          _results.push(Math.max.apply(Math, imageData.slice(i * this.width, (i * this.width + this.width) + 1 || 9e9)));
        }
        return _results;
      }).call(this);
      this.minimumPixel = Math.min.apply(Math, minsInRow);
      this.maximumPixel = Math.max.apply(Math, maxsInRow);
      sum = 0;
    ...
    AstroPNG.prototype.setVmin = function(vmin) {
      return this.vmin = vmin;
    };

    AstroPNG.prototype.setVmax = function(vmax) {
