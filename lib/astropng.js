(function() {
  ...
  AstroPNG = (function() {
    ...
    AstroPNG.random_number_generator = function(n) {
      var a, i, m, random_numbers, seed, temp;
      a = 16807.0;
      m = 2147483647.0;
      seed = 1;
      random_numbers = [];
    ...
    */
    ...
    AstroPNG.prototype.read_quantization_parameters = function(length) {
      ...
      return this.random_numbers = AstroPNG.random_number_generator(this.width * this.height);
    };
    ...
    AstroPNG.prototype.read_nan_locations = function(length) {
      ...
      length /= 2;
      this.y_nan = nan_locations.slice(0, length);
      return this.x_nan = nan_locations.slice(length);
    };
    ...
    AstroPNG.prototype.read_line = function() {
      var a, a_param, b, c, data, data_integer, element, filter_code, index, indices, r, recon_data, scale, zero, _i, _len, _len2, _ref;
      a_param = (function() {
        var _ref, _results;
        _results = [];
      ...
      if (typeof this.quantization_parameters !== 'undefined') {
        zero = this.quantization_parameters[2 * this.current_line];
        scale = this.quantization_parameters[2 * this.current_line + 1];
        r = this.random_numbers.slice(this.current_line * this.width, this.current_line * this.width + this.width);
        ...
        data = (function() {
          var _ref2, _results;
          _results = [];
          for (index = 0, _ref2 = data_integer.length - 1; 0 <= _ref2 ? index <= _ref2 : index >= _ref2; 0 <= _ref2 ? index++ : index--) {
            _results.push((data_integer[index] - r[index] + 0.5) * scale + zero);
          }
          return _results;
        })();
        indices = [];
        index = this.y_nan.indexOf(this.current_line);
        while (index !== -1) {
          indices.push(index);
          index = this.y_nan.indexOf(this.current_line, index + 1);
        }
        for (_i = 0, _len2 = indices.length; _i < _len2; _i++) {
          index = indices[_i];
          data[this.x_nan[index]] = Number.NaN;
        }
      } else {
        data = (function() {
          var _ref2, _ref3, _results;
    ...
    AstroPNG.prototype.read_image_data = function() {
      var j, _ref, _results;
      this.image_data = [];
      _results = [];
      for (j = 1, _ref = this.height; 1 <= _ref ? j <= _ref : j >= _ref; 1 <= _ref ? j++ : j--) {
        _results.push(this.image_data = this.image_data.concat(this.read_line()));
      }
      return _results;
    };

    AstroPNG.prototype.compute_statistics = function() {
      var diff2, image_data, pixel, sum, value, _i, _j, _k, _len, _len2, _len3, _ref;
      if (!(this.image_data != null)) this.read_image_data();
      image_data = [];
      _ref = this.image_data;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        pixel = _ref[_i];
        if (!isNaN(pixel)) image_data.push(pixel);
      }
      this.minimum_pixel = Math.min.apply(Math, image_data);
      this.maximum_pixel = Math.max.apply(Math, image_data);
      sum = 0;
      for (_j = 0, _len2 = image_data.length; _j < _len2; _j++) {
        pixel = image_data[_j];
        sum += pixel;
      }
      this.mean = sum / image_data.length;
      diff2 = 0;
      for (_k = 0, _len3 = image_data.length; _k < _len3; _k++) {
        value = image_data[_k];
        diff2 += (value - this.mean) * (value - this.mean);
      }
      diff2 /= image_data.length;
      return this.std = Math.sqrt(diff2);
    };

    AstroPNG.prototype.draw_on_canvas = function(context, vmin, vmax, stretch) {
      return console.log('draw_on_canvas');
    };
